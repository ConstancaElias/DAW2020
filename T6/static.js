...
function recursoEstatico(request){
    return /\/w3.css$/.test(request.url) || 
                /\/style.css$/.test(request.url) || 
                /\/frontEnd.js$/.test(request.url) || 
                /\/listIcon.ico$/.test(request.url) ||
                /\/trash.ico$/.test(request.url) ||
                /\/check.png$/.test(request.url) ||
                /\/pencil.ico$/.test(request.url)
